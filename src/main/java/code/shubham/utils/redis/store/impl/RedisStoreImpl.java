package code.shubham.utils.redis.store.impl;

import code.shubham.utils.redis.exception.RedisCacheException;
import code.shubham.utils.redis.exception.enums.RedisCacheErrorCodes;
import code.shubham.utils.redis.factories.JedisFactory;
import code.shubham.utils.redis.store.RedisStore;
import code.shubham.utils.redis.store.scripts.Constants;
import code.shubham.utils.redis.store.scripts.LuaScript;
...
public class RedisStoreImpl implements RedisStore {
    ...
    public void sadd(String key, String value) {
        ...
        try {
            ...
            log.error("Redis:sadd | Error Cache sadd | Key: {} | Value: {}", key, value, e);
            throw new RedisCacheException(RedisCacheErrorCodes.SAVE, e);
        } finally {
            jedisFactory.returnConnection(jedis);
        }
    ...
    @Override
    public List<Object> executeTransaction(List<Function<Transaction, Object>> commands) {
        List<Object> result = null;
        Jedis jedis = jedisFactory.getRedisConnection();
        try {
            Transaction transaction = jedis.multi();
            commands.forEach(function -> function.apply(transaction));
